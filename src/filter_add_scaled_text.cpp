...
filter_add_scaled_text::filter_add_scaled_text(const std::string & what, const std::string & font_file, const int x, const int y, const int font_size, const int width, const std::optional<rgb_t> bg, const rgb_t col, const bool invert, const std::map<std::string, feed *> & text_feeds) : what(what), font_file(font_file), x(x), y(y), font_size(font_size), width(width), bg(bg), col(col), invert(invert), text_feeds(text_feeds)
{
}
